...
```{r}
mouse_df_treat_long <- mouse_df_treat |> 
...
This is not this code, but here we can quickly get the mean across the repetions.
```{r}
mouse_df |> # Get the mean across the repetitions
    group_by(treatment) |> 
    summarise(across(-c(replicate), mean, na.rm = TRUE))
