...
human_aug_log <- read_tsv(file = "../data/03_dat_aug_human_loc.tsv.gz")
...
average_activity_treatment_replicate <- average_activity_treatment_replicate %>%
  mutate(treatment_replicate = paste(treatment, replicate, sep = " - "))

# Plot average activity for each treatment and replicate as a grouped bar plot with color by treatment
ggplot(average_activity_treatment_replicate, aes(x = treatment_replicate,
                                                 y = average_activity,
                                                 fill = treatment)) +
  geom_bar(stat = "identity", 
...
# DESC_3
human_aug_log |> 
  ggplot(mapping = aes(x = log2_scaled_expression_levels))+ 
  geom_freqpoly()+ 
  facet_wrap(~treatment)
```
...
human_aug_log |> 
  ggplot(mapping = aes(x = cell_line,
                       y = log2_scaled_expression_levels,
                       color = treatment))+
  geom_boxplot()+
   theme(axis.text.x = element_text(angle = 45), 
        axis.text = element_text(vjust = 0.5, 
