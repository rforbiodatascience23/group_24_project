...
human_aug_wide_gene <- read_tsv(file = "../data/03_dat_aug_human_wide_gene.tsv.gz")
...
knitr::kable(treatment_counts) 
```

```{r}
knitr::kable(replicate_counts)
```
...
The bulk of zeros suggests that there is no difference in expression between the different conditions. This is a common observation for an RNA Seq dataset, since many genes are not activated remain inactive or unaffected by the studied conditions.
...
ggplot(average_activity_treatment_replicate,
...
These are also very similar. It would be hard to know the treatment just from the activity.
...
mouse_zero_plot
ggsave("../results/mouse_zero_plot.png", bg = "white", device = "png")
```

We can see that most genes are either all zeroes (\~40%) or all non-zeroes( 33%).
...
As last part of our EDA we are going to Create a Venn Diagram in order to get a first clue of the genes that were commonly influenced for all the samples.
...
#selected_treatments <- names(active_genes_sets)[1:length(active_genes_sets)]
...
```{r}
knitr::kable(human_DMSO_zero_table)
```

```{r}
knitr::kable(human_g007_zero_table)
```
...
zero_counts_genes |> 
  ...
  labs(title = "Inactive Gene Percentage per Sample", 
       x = "Number of Samples", 
       y = "Percentage of Genes")
ggsave("../results/human_zero_plot.png", bg = "white", device="png")
```
