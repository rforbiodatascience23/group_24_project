...
```{r}
human_data_clean_wide2 <- human_data_clean_long |> 
  pivot_wider(
    names_from = Treatment,
    values_from = Expression_levels
  )

human_data_clean_wide2
```

We are going to merge our clean human datasets
...
Thinks to Keep in mind. Our data have been already normalized with the TPM method when the researchers used the sleuth v0.29 in their expreriment.
...
Estimate the relative expression of the genes (Actually that one is a way to reduce our 0s ):

following the formula

```{r}
human_data_transform<-human_data_trans_long |> 
  pivot_wider(
    names_from = treatment,
    values_from = Expression_levels
  )
human_data_transform
```

This is the log2fold transformation for the genes between the two compounds

```{r}
human_data_transform1<- human_data_transform |> 
  mutate(
...
```{r}
human_data_transform1 |> 
  ggplot(mapping = aes(x = log2fold_expression))+
  geom_freqpoly()
```

From that illustration we can see that there is not a global upregulation or down regulation by the gene. This is very common in RNA-seq arrays where most genes are not affected by the usage of a gene.
