...
human_joined_aug <- human_joined |> 
  mutate(norm_expression_levels =(Expression_levels + 0.99 * min_pos_value) 
         ) |> 
  select(-Expression_levels)
...
human_joined_aug_wide <- human_joined_aug |> 
  pivot_wider(
    ...
    values_from = norm_expression_levels
  )

human_joined_aug_wide <- human_joined_aug_wide |> 
  mutate(log2DMSO = round(log2(DMSO),2),
         log2G007.LK = round(log2(G007.LK),2))
