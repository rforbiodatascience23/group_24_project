...
```{r}
#lets replace the zero values in numeric columns with 0.65 * minimum positive value in every particular row
zeroes_replaced <- mouse_zeroes_removed |>  
  rowwise() |> #perform this in every row 
  mutate(
    across(
      everything(),
      ~ ifelse(. == 0, 0.65 * min(
        c_across(
          where(
            is.numeric))[c_across(where(is.numeric)) != 0], 
        na.rm = TRUE), .)))
...
```{r}
mouse_scaled_joined <-bind_cols(mouse_clean[1:2], scaled_data)
```
...
```{r}
human_zeroes_replaced <- human_zeroes_removed |>  
  rowwise() |> #perform this in every row 
  mutate(
    across(
      everything(),
...
human_zeroes_replaced_sums <- human_zeroes_replaced %>%
  rowwise() %>%
  mutate(total_sum = sum(c_across(everything()), na.rm = TRUE))
...
human_scaled_data <- human_zeroes_replaced_sums %>%
  mutate(scaling_factor = n / total_sum) %>%
  rowwise() %>%
  mutate(across(-c(total_sum, scaling_factor), ~ . * scaling_factor)) %>%
  select(-total_sum, -scaling_factor)
...
human_scaled_joined <- human_clean %>%
  select(1:2) %>%
  bind_cols(select(human_clean, 1:2), human_scaled_data)
```
