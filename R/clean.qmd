...
The join here is not working we are going to perform it with the mouse data####
...
human_suppl_clean_trans<- human_suppl_clean |> 
...
```{r}
human_suppl_clean_trans
```

Check for consistency in Cell_lines between the human datasets
...
```{r}
human_suppl_final <- human_suppl_clean_trans |> 
