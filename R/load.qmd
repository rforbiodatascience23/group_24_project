...
We study an RNA -seq RNA study perfomed on skin cells human and mice
...
### Create the supplementary files using the URLs
...
```{r}
human_sup_data<-read.table(str_c(raw_dir, human_sup_file), header = TRUE, sep = "\t")
mouse_sup_data<-read.table(str_c(raw_dir, mouse_sup_file), header = TRUE, sep = "\t")
```
